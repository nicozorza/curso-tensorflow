...
# To train our model, we'll use the LinearRegressor interface provided by the TensorFlow Estimator API. This API takes
# care of a lot of the low-level model plumbing, and exposes convenient methods for performing model training,
...
#   - Numerical Data: Data that is a number (integer or float) and that you want to treat as a number. As we will
#       discuss more later sometimes you might want to treat numerical data (e.g., a postal code) as if it were
#       categorical.
...
# To be safe, we also apply gradient clipping to our optimizer via clip_gradients_by_norm. Gradient clipping ensures
...
# to construct a dataset object from our data, and then break our data into batches of batch_size, to be repeated for
...
# Next, if shuffle is set to True, we'll shuffle the data so that it's passed to the model randomly during training. T
# he buffer_size argument specifies the size of the dataset from which shuffle will randomly sample.
...
# in my_feature and target as arguments (see this TensorFlow input function tutorial for more details), and to start,
# we'll train for 100 steps.
...
# generalizes to new data. In later exercises, you'll explore how to split your data to evaluate your model's ability
# to generalize.
...
# different parameters to see the effect.
# In this function, we'll proceed in 10 evenly divided periods so that we can observe the model improvement at
# each period.
# For each period, we'll compute and graph training loss. This may help you judge when a model is converged, or if it
# needs more iterations.
# We'll also plot the feature weight and bias term values learned by the model over time. This is another way to see
# how things converge.
